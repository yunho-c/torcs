...
; Upgrade-detection state across registry views.
; Existing*   : install path / uninstall command found in a specific view.
; Detected*   : selected install to uninstall during upgrade flow.
; Found32/64  : flags indicating whether an install was detected in each view.
; ProgramFiles64Dir: resolved 64-bit Program Files root (for default install target).
Var Existing32Dir
Var Existing32Uninst
Var Existing64Dir
Var Existing64Uninst
Var DetectedInstDir
Var DetectedUninst
Var DetectedRegView
Var Found32
Var Found64
Var ProgramFiles64Dir
...
SetOutPath "$INSTDIR\data\music"
  ...
  File "base\ogg.dll"
  File "base\vorbis.dll"
  File "base\vorbisfile.dll"
...
; Initialize installer state and force a 64-bit default install target.
; Resolution order:
;  1) ProgramW6432 environment variable
;  2) HKLM64 ProgramFilesDir
;  3) $PROGRAMFILES fallback
Function .onInit
  ReadEnvStr $ProgramFiles64Dir "ProgramW6432"
  StrCmp $ProgramFiles64Dir "" 0 +3
    SetRegView 64
    ReadRegStr $ProgramFiles64Dir HKLM "Software\Microsoft\Windows\CurrentVersion" "ProgramFilesDir"
    SetRegView 64
  StrCmp $ProgramFiles64Dir "" +3
    StrCpy $INSTDIR "$ProgramFiles64Dir\torcs"
    Goto +2
  StrCpy $INSTDIR "$PROGRAMFILES\torcs"
...
; Decide maintenance mode and migration target when an older install is present.
;
; Register usage in this function:
;  $R0: app-path value / version number scratch value
;  $R2: ProgramFiles(x86) path for prefix checks
;  $R3: length of $R2
;  $R4: prefix slice of Existing64Dir
;
; Behavior notes:
;  - Probe both HKLM64 and HKLM32 for TORCS install metadata.
;  - If both 32-bit and 64-bit installs are found, prefer uninstalling 32-bit.
;  - Never keep Program Files (x86) as default target for the new 64-bit install.
Function PageReinstall

  StrCpy $Found32 "0"
  StrCpy $Found64 "0"
  StrCpy $Existing32Dir ""
  StrCpy $Existing32Uninst ""
  StrCpy $Existing64Dir ""
  StrCpy $Existing64Uninst ""
  StrCpy $DetectedInstDir ""
  StrCpy $DetectedUninst ""
  StrCpy $DetectedRegView ""

  ; Resolve 64-bit Program Files for new-install default directory.
  ReadEnvStr $ProgramFiles64Dir "ProgramW6432"
  StrCmp $ProgramFiles64Dir "" 0 +3
    SetRegView 64
    ReadRegStr $ProgramFiles64Dir HKLM "Software\Microsoft\Windows\CurrentVersion" "ProgramFilesDir"

  ; Read existing install from the 64-bit registry view.
  SetRegView 64
  ReadRegStr $R0 HKLM "${PRODUCT_DIR_REGKEY}" ""
  StrCmp $R0 "" detect_32
    StrCpy $Found64 "1"
    ${GetParent} "$R0" $Existing64Dir
    ReadRegStr $Existing64Uninst ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString"

  detect_32:
  ; Read legacy install from the 32-bit registry view.
  SetRegView 32
  ReadRegStr $R0 HKLM "${PRODUCT_DIR_REGKEY}" ""
  StrCmp $R0 "" set_install_dir
    StrCpy $Found32 "1"
    ${GetParent} "$R0" $Existing32Dir
    ReadRegStr $Existing32Uninst ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString"

  set_install_dir:
  ; Keep current 64-bit install dir if present, unless it is under Program Files (x86).
  ; This avoids proposing x86 as target for the new 64-bit install.
  StrCmp $Found64 "1" 0 use_default_install_dir
    ReadEnvStr $R2 "ProgramFiles(x86)"
    StrCmp $R2 "" use_existing64 +6
      StrLen $R3 $R2
      StrCpy $R4 $Existing64Dir $R3
      StrCmp $R4 $R2 use_default_install_dir use_existing64

  use_existing64:
    StrCpy $INSTDIR $Existing64Dir

  StrCmp $Found64 "1" select_uninstall_target

  use_default_install_dir:
    ; No usable existing 64-bit path: default to resolved 64-bit Program Files.
    StrCmp $ProgramFiles64Dir "" +3
      StrCpy $INSTDIR "$ProgramFiles64Dir\torcs"
      Goto select_uninstall_target
    StrCpy $INSTDIR "$PROGRAMFILES\torcs"

  select_uninstall_target:
  ; If both installs exist, uninstall 32-bit first (migration policy).
  StrCmp $Found32 "1" 0 +5
    StrCpy $DetectedInstDir $Existing32Dir
    StrCpy $DetectedUninst $Existing32Uninst
    StrCpy $DetectedRegView "32"
    Goto other_version_present

  StrCmp $Found64 "1" 0 no_version_present
    StrCpy $DetectedInstDir $Existing64Dir
    StrCpy $DetectedUninst $Existing64Uninst
    StrCpy $DetectedRegView "64"
    Goto other_version_present
  ...
  StrCmp $DetectedRegView "64" 0 +2
    SetRegView 64
  StrCmp $DetectedRegView "32" 0 +2
    SetRegView 32
...
; Execute the selected maintenance action from the custom reinstall page.
;
; Register usage in this function:
;  $R0: mode set in PageReinstall ("1" = install path, "2" = maintenance path)
;  $R1: radio button state read from torcs.ini
;
; Uninstall flow:
;  - Try old uninstaller first (registry command, then uninst.exe fallback).
;  - If that fails, run conservative fallback cleanup:
;      * remove TORCS shortcuts
;      * clean TORCS uninstall/app-path keys from HKLM64 and HKLM32
;      * do NOT recursively delete old install directory (preserves user files)
Function PageLeaveReinstall
  ...
  reinst_uninstall:
  ; Run old uninstaller first to preserve original uninstall behavior.
  HideWindow

    ClearErrors
    StrCmp $DetectedUninst "" use_uninst_exe
      DetailPrint "Running old uninstaller command from registry..."
      ExecWait '$DetectedUninst _?=$DetectedInstDir'
      Goto check_uninstall_result

    use_uninst_exe:
    IfFileExists "$DetectedInstDir\uninst.exe" 0 check_uninstall_result
      DetailPrint "Running old uninstaller executable..."
      ExecWait '"$DetectedInstDir\uninst.exe" _?=$DetectedInstDir'

    check_uninstall_result:

    IfErrors direct_cleanup
    IfFileExists "$DetectedInstDir\wtorcs.exe" no_remove_uninstaller

      Delete "$DetectedInstDir\uninst.exe"
      RMDir "$DetectedInstDir"
      Goto no_remove_uninstaller

    direct_cleanup:
      DetailPrint "Old uninstaller failed, doing conservative cleanup only..."
      Delete "$SMPROGRAMS\TORCS\Track Editor.lnk"
      Delete "$SMPROGRAMS\TORCS\Track Editor Manual.lnk"
      Delete "$SMPROGRAMS\TORCS\TORCS User Manual.lnk"
      Delete "$SMPROGRAMS\TORCS\Frequently Asked Questions.lnk"
      ...
      ; Cleanup keys in both registry views to avoid stale entries.
      SetRegView 64
      DeleteRegKey ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}"
      DeleteRegKey HKLM "${PRODUCT_DIR_REGKEY}"
      SetRegView 32
